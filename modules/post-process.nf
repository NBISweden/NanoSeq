process POST_PROCESS {
	...
	# Perform logic check that all files have been correctly received
		...
		NUMBER_COV_BED=\$(ls *.varCov.bed.gz 2>/dev/null | wc -l || true)
		...
		if [[ "\$FILES_EXPECTED" -ne "\$NUMBER_COV_BED" || "\$FILES_EXPECTED" -ne "\$NUMBER_DISCARDED_VAR" || "\$FILES_EXPECTED" -ne "\$NUMBER_INDEL_FILTERED" || "\$FILES_EXPECTED" -ne "\$NUMBER_INDEL_FILTERED_TBI" || "\$FILES_EXPECTED" -ne "\$NUMBER_VAR" ]]; then
			echo "ERROR: Expected \$FILES_EXPECTED of each file type. Found \$NUMBER_COV_BED varCov.bed.gz files, \$NUMBER_DISCARDED_VAR discarded_var files, \$NUMBER_INDEL_FILTERED indel.filtered.vcf.gz files, \$NUMBER_INDEL_FILTERED_TBI indel.filtered.vcf.gz.tbi files, and \$NUMBER_VAR var files."
			exit 1
		fi
